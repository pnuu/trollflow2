...
class Runner:
    ...
    def _run_product_list_on_messages(self, messages, target_fun, process_class):
        ...
        for msg in messages:
            ...
            kwargs = dict(produced_files=produced_files_queue,
                          prod_list=self.product_list, log_config=self.log_config)
...
def launch(args_in):
    ...
    with logging_on(log_queue, log_config):
        ...
        runner = Runner(product_list, log_queue, connection_parameters, test_message, threaded, log_config=log_config)
        runner.run()
